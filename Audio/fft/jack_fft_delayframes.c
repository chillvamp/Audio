...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	for(i = 0; i < nframes; i++){
		aux=b_index-nframes+i-(int)(delay);
		printf ( " aux %i  ",aux);
		if (aux >=0 ){
		printf("aux %i",aux);
		out [i]= buff_b[aux];
		...
		if (aux <0  ){
		printf("aux %i",4*nframes+aux);
		out [i]= buff_b[4*nframes+aux];
		}
...
int main (int argc, char *argv[]) {
	...
	freqs=  (double*) malloc (sizeof(double)*4*nframes);
	buff_a=(jack_default_audio_sample_t*) malloc (sizeof(jack_default_audio_sample_t)*4*nframes);
	buff_b=(jack_default_audio_sample_t*) malloc (sizeof(jack_default_audio_sample_t)*4*nframes);
	


	for (int i=0;i<2*nframes;++i){
		freqs[i]= i*(sample_rate/nframes)/4;
		freqs [4*nframes - i]= -freqs[i];
	}
	freqs[2*nframes]= nframes*(sample_rate/nframes);
	printf("MINFRE: %f; max Freq: %f\n",freqs[1], freqs[2*nframes]);
	hann=(double*) malloc (sizeof(double)*4*nframes);
	for (int i=0;i< 4*nframes;++i){
		
		hann[i]= .5*(1-cos(2*M_PI*i/4/nframes));

		
	}
	//preparing FFTW3 buffers
	i_fft = (double complex *) fftw_malloc(sizeof(double complex) *4* nframes);
	i_time = (double complex *) fftw_malloc(sizeof(double complex) *4* nframes);
	o_fft = (double complex *) fftw_malloc(sizeof(double complex) *4* nframes);
	o_time = (double complex *) fftw_malloc(sizeof(double complex) *4*nframes);
	
	i_forward = fftw_plan_dft_1d(4*nframes, i_time, i_fft , FFTW_FORWARD, FFTW_MEASURE);
	o_inverse = fftw_plan_dft_1d(4*nframes, o_fft , o_time, FFTW_BACKWARD, FFTW_MEASURE);
