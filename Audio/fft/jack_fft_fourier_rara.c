...
jack_default_audio_sample_t *buff_a ,*buff_b,*buff_c ;
double sample_rate;
double *freqs;
double *hann;
double complex aux;
...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	for(i = 0; i < nframes; i++){
		...
		if (i< nframes/2){
				//printf("pendejo,%f\n",buff[i]);
				i_time[i]= buff_b[i] * hann[i] + hann[nframes/2+i]*buff_a[nframes/2+i]; 
				buff_c[i]=buff_b[i+nframes/2];	
				buff_b[i]= in[i];
				buff_a[i]=buff_c[i];	
				//printf("%f\n",buff[i]);
		}
			...
			//printf("%f,%f \n",in[i],in[i- nframes/2] );
			i_time[i]=  buff_b[i] * hann[i] +  buff_c[i- nframes/2] *hann[i-nframes/2];
			buff_c[i]=in[i-nframes/2];	
			buff_b[i]= in[i];
			buff_a[i]=buff_c[i];
		}
